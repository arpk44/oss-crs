...
# Make incremental build snapshots for all JVM projects and optionally push to registry
...
usage() {
    ...
    echo "  $0 /path/to/oss-fuzz --rts-tool jcgeks --push both"
    exit 1
}
...
LOG_DIR="$OSS_CRS_DIR/logs/jvm_inc_snapshot_$(date +%Y%m%d_%H%M%S)"
RESULT_DIR="$LOG_DIR/.results"
mkdir -p "$LOG_DIR" "$RESULT_DIR"

echo "=========================================="
echo "JVM Incremental Build Snapshot Maker"
echo "=========================================="
echo "OSS-Fuzz path: $OSS_FUZZ_PATH"
echo "JVM projects:  $JVM_PROJECTS_DIR"
...
run_single_snapshot() {
    ...
    if uv run oss-bugfix-crs make-inc-snapshot "aixcc/jvm/$project" "$oss_fuzz_path" $cmd_opts > "$log_file" 2>&1; then
...
if [ "$JOBS" -eq 1 ]; then
    ...
    for project in "${projects[@]}"; do
        current=$((current + 1))
        echo "[$current/$total] Making snapshot: aixcc/jvm/$project"
...
JVM Incremental Build Snapshot Summary
=======================================
Date: $(date)
Parallel Jobs: $JOBS
